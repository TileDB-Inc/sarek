...
// We can't use dict as a file as it's made of an index, and it's already the complete path
dict               = params.dict                        ? Channel.value(params.dict)                                   : Channel.empty()

// Initialize channels for optional files
ch_ascat_alleles = params.ascat_alleles ? Channel.fromPath(params.ascat_alleles).collect() : Channel.value([])
ch_ascat_loci    = params.ascat_loci    ? Channel.fromPath(params.ascat_loci).collect()    : Channel.value([])
ch_ascat_loci_gc = params.ascat_loci_gc ? Channel.fromPath(params.ascat_loci_gc).collect() : Channel.value([])
ch_ascat_loci_rt = params.ascat_loci_rt ? Channel.fromPath(params.ascat_loci_rt).collect() : Channel.value([])

//Adding resources for cnvkit
cnvkit_reference = params.cnvkit_reference ? Channel.fromPath(params.cnvkit_reference).collect() : Channel.value([])

// Adding resources for ControlFreeC
chr_files = params.chr_files ? Channel.fromPath(params.chr_files).collect() : []
...
// snpeff_genome
if (params.snpeff_genome) {
    snpeff_genome = Channel.value(params.snpeff_genome)
} else snpeff_genome = Channel.empty()
